...
| **Criteria Corp** (Ch. 12) | Optional **AI proctoring** (no lockdown); design-based security | Dynamic item bank; session logs; **image capture/analysis + screen/audio sampling**; CCAT **"Invalid Result"** (prior-exposure via speed+accuracy); EPP validity/consistency scales | Flags → **employer decides**; Invalid Result can misfire on fast, accurate first-timers | Not told of a flag |
| **Korn Ferry / Talent Q** (Ch. 13) | **Unproctored typical**; low-surveillance by design | Adaptive item selection + **per-item clocks** as design security; integrity telemetry `[UNKNOWN]` — no verification re-test or webcam layer documented | n/a — little to flag; main honest risk is a **disconnection eating a per-item window** | n/a |
