...
def collect():
    ...
    for f in sorted(glob.glob(str(HERE/'*.html'))):
        for m in re.findall(r'\[(P\d+)\]', open(f,encoding='utf-8').read()): cited.add(m)
    rows=dict(EDITOR)
    for f in sorted(glob.glob(str(Q/'P*.md'))):
        for line in open(f,encoding='utf-8'):
            line=line.strip()
            m=re.match(r'^\[(P\d+)\]\s*\|(.*)$', line)
            if not m:
                # markdown-table form: | P150 | outlet | title | date | url | accessed | status | note |
                m2=re.match(r'^\|\s*(P\d+)\s*\|(.*)$', line)
                if not m2: continue
                m=m2
            sid=m.group(1)
            if sid in rows: continue
            p=[x.strip() for x in m.group(2).rstrip('|').split('|')]
...
if __name__ == '__main__':
    ...
    out=['<section class="part">\n<span class="marker">@@ANCHOR:sources@@</span>\n',
     '<div class="part-band"><p class="kicker">Part 13</p><h2 class="part-title">Source table</h2>',
    ...
    out.append('</tbody><caption>All URLs accessed 19 September 2026.</caption></table>\n</section>\n')
    (HERE/'80_sources.html').write_text(''.join(out), encoding='utf-8')
