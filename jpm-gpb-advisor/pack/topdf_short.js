...
(async () => {
  const [input, output, header, prepared] = process.argv.slice(2);
  ...
  await page.pdf({ path: output, format: 'A4', printBackground: true, preferCSSPageSize: true,
    ...
    headerTemplate: `<div style="font-size:7.5px;color:#8a93a3;width:100%;padding:0 17mm;font-family:Arial;display:flex;justify-content:space-between;"><span>${header}</span><span>${prepared}</span></div>`,
